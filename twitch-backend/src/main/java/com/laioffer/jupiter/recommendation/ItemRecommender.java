...
public class ItemRecommender {
    ...
    // Return a list of Item objects for the given type. Types are one of [Stream,
    // Video, Clip]. Add items are related to the top games provided in the argument
    private List<Item> recommendByTopGames(ItemType type, List<Game> topGames) throws RecommendationException {
        List<Item> recommendedItems = new ArrayList<>();
        TwitchClient client = new TwitchClient();

        outerloop: for (Game game : topGames) {
            List<Item> items;
            try {
    ...
    // Return a list of Item objects for the given type. Types are one of [Stream,
    // Video, Clip]. All items are related to the items previously favorited by the
    // user. E.g., if a user favorited some videos about game "Just Chatting", then
    // it will return some other videos about the same game.
    private List<Item> recommendByFavoriteHistory(Set<String> favoritedItemIds, List<String> favoriteGameIds,
            ItemType type) throws RecommendationException {
        // Count the favorite game IDs from the database for the given user. E.g. if the
        // favorited game ID list is ["1234", "2345", "2345", "3456"], the returned Map
        // is {"1234": 1, "2345": 2, "3456": 1}
        Map<String, Long> favoriteGameIdByCount = favoriteGameIds.parallelStream()
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));

        // Sort the game Id by count. E.g. if the input is {"1234": 1, "2345": 2,
        // "3456": 1}, the returned Map is {"2345": 2, "1234": 1, "3456": 1}
    ...
    // Return a map of Item objects as the recommendation result. Keys of the may
    // are [Stream, Video, Clip]. Each key is corresponding to a list of Items
    // objects, each item object is a recommended item based on the previous
    ...
    // Return a map of Item objects as the recommendation result. Keys of the may
    // are [Stream, Video, Clip]. Each key is corresponding to a list of Items
    // objects, each item object is a recommended item based on the top games
    // currently on Twitch.
